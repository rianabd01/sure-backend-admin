...
const putToDeleteTrash = async (request, h) => {
  ...
  try {
    ...
    if (trashItem.is_deleted === 1) {
      return h
        .response({
          status: 'fail',
          message: 'trash has already deleted',
        })
        .code(403);
    }
    ...
    // Delete the trash
    // eslint-disable-next-line camelcase
    trashItem.is_deleted = 1;
    await trashItem.save();

    return h.response({
      status: 'success',
      message: 'delete trash success',
    });
  } catch (error) {
    return h
      .response({
...
module.exports = putToDeleteTrash;
